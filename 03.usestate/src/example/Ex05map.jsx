...
const Ex05map = () => {
    ...
    // 1. map 함수 : 기존 배열을 이용해서 새로운 배열을 만드는 배열함수 (JS내장)
    //            : 기존 배열.map(콜백함수)
    // 2. filter 함수 : 기존 배열에서 조건을 이용해서 조건이 true면 유지, false면 버려서 새로운 배열을 만드는 배열 함수 (JS내장)
    //               : 기존배열.filter(콜백함수)
  ...
  return (
    <div>
        ...
        <p>
            안녕하세요 제 이름은 {students[0].name}입니다.
            저는 {students[0].bloodType}형이고, {students[0].age}살 입니다.
        </p>
        <p>
            안녕하세요 제 이름은 {students[1].name}입니다.
            저는 {students[1].bloodType}형이고, {students[1].age}살 입니다.
        </p>
        <p>
            안녕하세요 제 이름은 {students[2].name}입니다.
            저는 {students[2].bloodType}형이고, {students[2].age}살 입니다.
        </p>

        <h3>Case 2 : map함수를 통해서 출력</h3>
        {/* 기존의 배열(students)을 가져와서 결과를 반복 처리해서 새로운 배열을 반환
            기존의 배열은 영향을 받지 않는다
            key 값을 꼭 저장해야한다
            map 함수 : 배열이 가지고있는 데이터(요소) 한 번씩 접근해서
                      컴포넌트를 통해 출력하는 방법
        */}
        {students.map((item) => (
            ...
            <p key={item.name}>
                ...
                저는 {item.bloodType}형이고, {item.age}살 입니다.
            </p>
        ))}
        <h3>Case 3 : map함수 실습</h3>
        {/* food변수를 map함수를 통해서 => 버튼요소로 만들기 */}
        <p>
            {food.map((item) =>(
            <button style={{background : 'yellow'}} key={item}>{item}</button>
            ))}
        </p>
        {/* numList변수를 map함수를 통해서 => 버튼요소로 만들기
            => 2,4,6,8,10
        */}
        <p>
            {numList.map((item) => (
        ...
        {/* 
            1. numList배열을 map 함수를 통해서 => 버튼요소로 만들기
